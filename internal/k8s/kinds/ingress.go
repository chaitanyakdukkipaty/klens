...
func (ingress) Columns() []k8s.Column {
	return []k8s.Column{
		{Header: "NAME", Width: 35, Flex: true},
		{Header: "ADDRESSES", Width: 22},
		{Header: "RULES", Width: 40},
		{Header: "AGE", Width: 10},
	}
}

func (i ingress) List(c Context) ([]Row, error) {
	ings, err := listTyped[*networkingv1.Ingress](c, i.Meta().GVR, c.Namespace)
	if err != nil {
		return nil, err
	}
	rows := make([]Row, 0, len(ings))
	for _, ing := range ings {
		...
		rows = append(rows, Row{
			...
			Values:    []string{ing.Name, addrStr, rulesStr, k8s.AgeString(ing.CreationTimestamp)},
			Raw:       ing,
		})
	}
	return rows, nil
